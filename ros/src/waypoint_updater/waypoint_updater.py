...
numberOfWaypointsToPublish = 250
# Maximum speed of the car, in meters per second (mph * 0.44704 = speed in m/s)
maximumSpeed = 10*ONE_MPH
# Number of waypoints to slowdown, getting to a traffic light
numWayPointsDeaccelTF = 75

# Waypoint Updater Node Overview
# https://classroom.udacity.com/nanodegrees/nd013/parts/6047fe34-d93c-4f50-8336
# -b70ef10cb4b2/modules/e1a23b06-329a-4684-a717-ad476f0d8dff/lessons/462c933d-
...
class WaypointUpdater(object):
    def __init__(self):
        ...
        self.waypoints = None
        # Holds the current pose information (current_pose)
        self.currentPose = None
        # Holds the current vel. information (current_velocity)
        self.currentVelocity = None
        self.redLightWaypoint = -1
        # Holds the index for the closest waypoint from the vehicle
        self.closestWaypointIndex = None
        # Holds the state of the next traffic light
        self.nextTrafficLightState = None
        # Holds the waypoint index where the next traffic light is
        self.nextTrafficLightWaypointIndex = None
        rospy.spin()

    ''' currentVelocityCallback, callback function
        for current_velocity subscriber
    '''
    def currentVelocityCallback(self, msg):
        self.currentVelocity = msg

    ''' currentPoseCallback, callback function
        for current_pose subscriber
    '''
    def currentPoseCallback(self, msg):
        self.currentPose = msg.pose
        self.publishNextWaypoints()

    ''' baseWaypointsCallback, callback function for
        base_waypoints subscriber
    '''
    ...
    ''' trafficStateCallback, callback function for all_traffic_waypoint
        subscriber
    '''
    ...
    ''' publishNextWaypoints, function that uses the messages provided by
        the subscribed topics base_waypoints, current_pose and etc. To
        generate a list of waypoints and publish those to the final_waypoints
        topic (this is a Lane message)
    '''
    def publishNextWaypoints(self):
        ...
        ''' Create list for next waypoints for the car to follow,
        ...
        for i in range(len(nextWaypoints) - 1):
            ...
            if not isRedTrafficLightNear:
                ...
                ''' If current velocity of the car is considerable
                ...
                ''' Else, if the waypointToGo is not in a close range,
                    accelerate the car, saying that this waypoint can
                    run in maximum velocity
                '''
                else:
            ...
            # Else, if red traffic light is near the car
                ...
                ''' If the range of waypointToGo calculated is really
                    close to the car, < 4 index, stop the car
                '''
                if waypointToGo < 4:
                    newVelocity = 0.0
                ...
                ''' If the range is larger than 30, change velocity in a
                    constant pattern, linear calculation below for rate
                    of change of speed.
                '''
                else:
                ...
                ''' Impose the velocity calculated for the waypoint
                    iterated considering the scenarios above
                '''
